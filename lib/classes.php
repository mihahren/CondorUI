	...
	public function submitFile($files, $user_name, &$output)
	{
		if (is_array($files))
		{
			foreach ($files as $key => $value)
			{
				...
				{
					$output[$key] = "Datoteka ".$file_name['basename']." ve� ne obstaja!";
				}
				elseif(preg_match("/[^a-z0-9_.-]/i", $file_name['basename']))
				{
					$output[$key] = "Ime submitane datoteke lahko vsebuje samo �tevilke, �rke in znake _ . -";
				}
				else
				{
		...
		else
		{
			...
			{
				$output['submit'] = "Datoteka ".$file_name['basename']." ve� ne obstaja!";
			}
			elseif(preg_match("/[^a-z0-9_.-]/i", $file_name['basename']))
			{
